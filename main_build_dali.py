...
min_loss = 100.0  # the biggest is only 4.xx in total
max_cc = [0, 0]

notice='TID_dali_RECON-3_PERCP-1_320_e3e5'
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        is_best = (lcc + srocc) > (max_cc[0] + max_cc[1])
        max_cc = ([lcc, srocc] if is_best else max_cc)

        print('plcc:%.3f, srocc:%.3f [BEST]' % (max_cc[0], max_cc[1]))

        if epoch > 3 and is_best:
            print('inference')
        ...
        if (epoch+1)%10==0:
            ...
            plot(args.start_epoch+np.arange(len(plot_losses1)), plot_losses1)
            plot(args.start_epoch+np.arange(len(train_losses1)), train_losses1)
            ...
            plot(args.start_epoch+np.arange(len(plot_losses2)), plot_losses2)
            plot(args.start_epoch+np.arange(len(train_losses2)), train_losses2)
            # savefig(args.plot_dir + 'loss_ep%d_.jpg' % (epoch))
            # close()
            ...
            savefig(args.plot_dir + 'ep%d_.jpg' % (epoch))
            close()
...
def train(train_loader, val_loader, model, deconv_net, vgg, critition,mse_loss, optimizer, epoch):
    ...
    for i, (input, target, std) in enumerate(train_loader):
        ...
        features_y = vgg(y)
        features_x = vgg(x)
        # features_x = feats.relu3_3.detach()


        # loss2 = 10*mse_loss(features_y.relu2_2, features_x.relu2_2)
        loss2 = 10* mse_loss(features_y.relu1_2, features_x.relu1_2)


        loss = loss1 + loss3 + loss2 + loss4

        # loss = ((target-output).pow(2)*weight).mean()

        # loss = critition(output, target)

        losses1.update(loss1.item(), input.size(0))
        losses2.update(loss2.item(), input.size(0))
        losses.update(loss4.item(), input.size(0))
...
def validate(val_loader, model, deconv_net,vgg, critition,mse_loss,  epoch):
    ...
    for i, (input, target, std) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            # plt.imshow(new_img)
            new_img.save('./plot/%d%d.jpg'%(epoch,i))
        ...
        features_y = vgg(y)
        features_x = vgg(x)
        # loss2 = 10 *mse_loss(features_y.relu3_3, features_x.relu3_3)
        # loss2 = 10 *mse_loss(features_y.relu2_2, features_x.relu2_2)
        loss2 = 10  * mse_loss(features_y.relu1_2, features_x.relu1_2)
    ...
    return losses1.avg, losses2.avg, losses.avg, val_lcc, val_srocc
